...
const ProjectCostStatusModal = ({ isOpen, onClose, cost, onStatusChange }) => {
  ...
  const getStatusOptions = () => {
    if (!cost) return [];
    return VALID_TRANSITIONS[cost.status] || [];
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.put(
        `${process.env.NEXT_PUBLIC_API_URL}/api/projects/${cost.projectId}/costs/${cost.id}/status`,
        { 
          status: newStatus, 
          notes,
      ...
      if (response.data.success) {
        toast({
          ...
          description: `Status biaya proyek berhasil diubah ke ${newStatus}`,
          status: 'success',
          duration: 3000,
          isClosable: true,
        });
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Ubah Status Biaya Proyek</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          {cost && (
            <VStack spacing={4} align="stretch">
              <HStack>
                ...
                <Badge colorScheme={STATUS_COLORS[cost.status]} fontSize="0.9em" p={1}>
              ...
              {newStatus === 'rejected' && (
                <Box p={3} bg="red.50" borderRadius="md">
                  <Text fontSize="sm" color="red.600">
                    <strong>Perhatian:</strong> Mengubah status menjadi "Rejected" akan membatalkan semua jurnal yang terkait dengan biaya proyek ini. Tindakan ini tidak dapat dibatalkan.
                  </Text>
                </Box>
              )}
            </VStack>
...
export default ProjectCostStatusModal; 